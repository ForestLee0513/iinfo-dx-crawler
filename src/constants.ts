...
export const CONFIG: Config = {
  path: "/game/2dx/34/djdata/music/difficulty.html",
  statusPath: "/game/2dx/34/djdata/status.html", // 프로필(스테이터스) 페이지
  // 공식 CSV 다운로드 페이지. style=SP|DP 로 POST 하면 응답 HTML 의
  // textarea#score_data 에 전체 CSV 가 들어온다. (구독 미가입 시 error.html?err=5 로 리다이렉트)
  scorePath: "/game/2dx/34/djdata/score_download.html",
  // 베이직 코스 미구독 시 eagate 가 리다이렉트하는 에러 페이지(경로만 비교).
  // 예: /game/2dx/34/error/error.html?err=1
  errorPath: "/game/2dx/34/error/error.html",
  styles: [
    { name: "SP", style: 0 },
    { name: "DP", style: 1 },
  ],
